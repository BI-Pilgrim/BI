...
spend_b4_death,
-- case
-- when min(case when roas >= 0.8 and next_roas <= 0.8 then day_no end) is null then 0
-- else min(case when roas >= 0.8 and next_roas <= 0.8 then day_no end)
-- end as days2death_end,

-- case
-- when min(case when roas >= 0.8 and next_roas >= 0.8 then day_no end) is null then 0
-- else min(case when roas >= 0.8 and next_roas <= 0.8 then day_no end)
-- end as days2death_begin,
...
spend_b4_death,
roas,
LEAD(roas) OVER (partition by ad_id ORDER BY day_no) AS next_roas
from
(
  select
  ad_id,
  ...
  date_start,
  start_date,
  sum(daily_spend) over(partition by ad_id order by date_start) as spend_b4_death,

  case
  when (sum(daily_spend) over(partition by ad_id order by date_start)) = 0 then 0
  else (sum(daily_revenue) over(partition by ad_id order by date_start)) / (sum(daily_spend) over(partition by ad_id order by date_start)) 
  end as roas,
  ...
  (
    ...
    date_start,
    min(date_start) over(partition by ad_id order by date_start) as start_date,
    ...
    (
      ...
      where JSON_EXTRACT_SCALAR(act,'$.action_type') = 'purchase' --and ad_id = '120200086228210111'--'120200231086040111'
      AND REGEXP_CONTAINS(ad_name, r'\| (000|3|9)')
      AND NOT REGEXP_CONTAINS(LOWER(ad_name), r'\b(static|sale|collection|carousal|Static|Sale|Collection|Carousal|Sales|sales|offer|Offer)\b')
      -- order by ad_id,date_start
    )
...
order by ad_id, day_no
)
-- or (roas >= 0.8 and roas <= previous_roas)
group by ad_id,spend_b4_death--,start_date,date_start,day_no
order by ad_id--, date_start
)
-- where days2death_begin != 0 or days2death_end != 0
where days2death > 1
)
...
btd.best_tier,
case
when ald.days2death is null then 0
else ald.days2death
end days2death,

case
when ald.spend_b4_death is null then 0
else round(ald.spend_b4_death)
end as spend_b4_death,
concat(extract(year from start_date),'-',extract(month from start_date)) as start_period,

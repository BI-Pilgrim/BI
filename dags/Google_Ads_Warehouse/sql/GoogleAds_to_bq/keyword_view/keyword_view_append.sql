...
(
select *,
row_number() over(partition by ad_group_id,segments_date,ad_group_criterion_keyword_text,ad_group_criterion_keyword_match_type order by _airbyte_extracted_at desc) as rn
...
and TARGET.ad_group_criterion_keyword_text = SOURCE.ad_group_criterion_keyword_text
and TARGET.ad_group_criterion_keyword_match_type = SOURCE.ad_group_criterion_keyword_match_type
WHEN

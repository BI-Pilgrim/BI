CREATE OR REPLACE TABLE `shopify-pubsub-project.Data_Warehouse_Facebook_Ads_Staging.ads_insights_demographics_country_action_values`
...
select
*,
row_number() over(partition by ad_id,date_start,country,JSON_EXTRACT_SCALAR(act_values, '$.action_type') order by _airbyte_extracted_at) as rn

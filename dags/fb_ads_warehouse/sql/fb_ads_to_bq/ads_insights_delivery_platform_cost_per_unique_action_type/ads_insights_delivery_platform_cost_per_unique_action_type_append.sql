merge into `shopify-pubsub-project.Data_Warehouse_Facebook_Ads_Staging.ads_insights_delivery_platform_cost_per_unique_action_type` as target
using
(
SELECT
...
select
*,
row_number() over(partition by ad_id,date_start,publisher_platform,JSON_EXTRACT_SCALAR(cpuat, '$.action_type') order by _airbyte_extracted_at) as rn
...
target.cost_per_unique_action_type_28d_views = source.cost_per_unique_action_type_28d_views,
...
(
  ...
  cost_per_unique_action_type_28d_views,  
...
source.cost_per_unique_action_type_28d_views,

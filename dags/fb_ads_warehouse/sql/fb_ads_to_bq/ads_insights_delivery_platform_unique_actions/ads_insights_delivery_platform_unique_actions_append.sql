merge into `shopify-pubsub-project.Data_Warehouse_Facebook_Ads_Staging.ads_insights_delivery_platform_unique_actions` as target
using
(
SELECT
...
select
*,
row_number() over(partition by ad_id,date_start,publisher_platform,JSON_EXTRACT_SCALAR(unique_acts, '$.action_type') order by _airbyte_extracted_at) as rn
...
target.unique_actions_28d_views = source.unique_actions_28d_views,
...
source.unique_actions_28d_views,

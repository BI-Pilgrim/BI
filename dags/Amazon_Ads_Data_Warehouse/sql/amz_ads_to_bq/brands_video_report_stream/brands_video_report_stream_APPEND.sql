MERGE INTO `shopify-pubsub-project.Data_Warehouse_Amazon_ads.brands_video_report_stream` AS target
USING (
  SELECT 
  ...
  WHERE 
    _airbyte_extracted_at > (
      ...
      FROM `shopify-pubsub-project.Data_Warehouse_Amazon_ads.brands_video_report_stream`
    )
) AS source
ON FALSE  

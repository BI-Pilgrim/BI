...
from easy_com.return_orders.return_orders_schema import AllReturnOrders
...
class easyEComAllReturnOrdersAPI(EasyComApiConnector):
    def __init__(self):
        super().__init__()
        self.url = self.base_url + "/orders/getAllReturns"
        self.project_id = "shopify-pubsub-project"
        self.dataset_id = "easycom"
        self.name = "Return Orders"
        self.table = AllReturnOrders
    ...
    def transform_data(self, data):
        ...
        for record in data:
            transformed_record = {
                ...
                "credit_note_date": datetime.strptime(record["credit_note_date"], "%Y-%m-%d %H:%M:%S") if record.get("credit_note_date") else None,
            }
    ...
    def get_data(self):
        ...
        while next_url:
            ...
            try:
                ...
                all_returns = data.get("data") or {}
                table_data.extend(all_returns.get('credit_notes', []))

...
@dag("sync_orders", schedule='0 1 * * *', start_date=datetime(year=2024,month=1,day=1), tags=["easyecom", "report"])
def sync_orders():
    # order data sync
    @task.python
    def sync():
...
@dag("sync_grn_details", schedule='0 2 * * *', start_date=datetime(year=2024,month=1,day=1), tags=["easyecom", "report"])
...
@dag("sync_inventory_details", schedule='0 2 * * *', start_date=datetime(year=2024,month=1,day=1), tags=["easyecom", "report"])
...
@dag("sync_purchase_orders", schedule='0 2 * * *', start_date=datetime(year=2024,month=1,day=1), tags=["easyecom", "report"])
